...
lr=2e-5
epochs=2
micro_batch_size=4 # -> batch_size will be 64 if 8 gpus, per device batch size in single node
...
accelerate launch --config_file=train/deepspeed_zero3.yaml \
    ...
    --output_dir training_outputs/sft-llama-3.2-11b-vision-instruct-${uid} \
    --bf16 True \
