...
if __name__ == "__main__":
    ...
    # 1. Load the data as a streaming IterableDataset
    # This avoids loading the whole file into memory. It reads line by line as needed.
    logging.info(f"Loading data from {file_path} in streaming mode.")
    streaming_dataset = load_dataset(
        "json",                # The format is json-lines
        data_files=file_path,
        split="train",         # HF requires a split name
        streaming=True
    )

    # You can inspect the first item by creating an iterator and calling next()
    first_sample = next(iter(streaming_dataset))
    logging.info(f"Loaded dataset in streaming mode. First sample: {first_sample}")

    # 2. Process the dataset in a streaming fashion using .map()
    # The .map() function on an IterableDataset returns another IterableDataset,
    # creating a processing pipeline without loading everything into memory.
    logging.info("Applying 'process_example_local' transformation in a streaming fashion.")
    processed_streaming_dataset = streaming_dataset.map(process_example_local)

    # 3. Verify the types of the first few samples
    # You can't index a streaming dataset (e.g., processed_streaming_dataset[0]).
    # Instead, you iterate over it. `itertools.islice` is perfect for this.
    print("🔍 Verifying image types from the streaming dataset...")
    # Get the first 3 items from the iterator for verification
    for i, sample in enumerate(itertools.islice(processed_streaming_dataset, 3)):
        # Assuming 'images' is a list and contains at least one image
        if 'images' in sample and sample['images']:
            img = sample['images'][0]
            print(f"Sample {i}: {type(img)}")
            # Optional: Stronger check
            assert isinstance(img, PILImage), f"Image in sample {i} is not a PIL.Image, but {type(img)}"
        else:
            print(f"Sample {i} has no images to verify.")

    logging.info("Streaming dataset pipeline is ready for training.")
